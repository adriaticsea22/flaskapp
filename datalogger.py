...
def log_data():
    ...
    data_string = 'uv-index,device=veml,location=inside value={}'.format(UV_index)
    r = requests.post(url_string, data=data_string)
    aio.send('uv-a', UV_A)
    aio.send('uv-b', UV_B)
    aio.send('lux', light_level)
    data_string = 'light,device=lux,location=inside value={}'.format(light_level)
    r = requests.post(url_string, data=data_string)
    ...
    data_string = 'VOC,device=gassensor,location=inside value={}'.format(gas.value)
    r = requests.post(url_string, data=data_string)
